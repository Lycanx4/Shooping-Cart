...
namespace Team2_DotNetCA.Controllers
{
    public class CartController : Controller
    {
        ...
        public IActionResult ViewCart(int userId)
        {
            ...
            if (sessionId == null)
            {
                // Matt's cookies kvp --> productId : numOfPrd

                Cart cart_guest = new Cart();

                for (int i = 1; i <= 6; i++) //filter for integer (sessionId)
                {
                    string? quantity = Request.Cookies[i.ToString()];

                    if (quantity == null) // product not found in cookies
                        continue;
                    else // product is found in cookies
                    {
                        int productId = i;
                        ...
                        // (Dictionary<Product, int>) CartItems
                    }
                }
            ...
            /* TESTING DATA
            Cart cart1 = CartData.GetCartItems(1);
        ...
        public IActionResult UpdateCartCookies(int prodId, int qty) // JS.pdf slide 45 -> parameter names must be same as json obj names for name/value pairs
        {
            if (qty == 0)
            {
        ...
        public IActionResult UpdateCartDB(int uId, int prodId, int qty) // JS.pdf slide 45 -> parameter names must be same as json obj names for name/value pairs
        {
            // check if user has logged in ==> sessionId in cookies = sessionId in DB
            string sessionId = Request.Cookies["sessionId"];
